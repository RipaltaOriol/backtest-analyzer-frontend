...
const UpdateDocument = () => {
    ...
    if (documentObj && documentObj.state && Object.keys(documentObj.state).length !== 0) {
    ...
    return (
        <Box sx={{ display: 'block', width: 'calc(100% - 200px)'}}>
            <Box>
                <Typography variant='h5'>{document ? document.name : 'Loading'}</Typography>
            </Box>
            ...
            <Box sx={{ display: 'flex', width: '100%', mb: 3 }}>
                <Box sx={{ flexGrow: 1 }}>
                    <Typography variant="h6" sx={{ mb: 2}}>
                        { Object.keys(selectedRow).length === 0 ? 'New Item' : 'Update Item' }
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                    <Button
                        variant='contained'
                        onClick={() => {
                            handleDocumentUpdate(Object.keys(selectedRow).length === 0 ? 'add' : 'update')
                        }}
                    >
                        { Object.keys(selectedRow).length === 0 ? 'Add' : 'Update' }
                    </Button>
                    { Object.keys(selectedRow).length !== 0 &&
                        <Button variant='contained' sx={{ mt: 1 }} onClick={() => handleDocumentUpdate('delete')}>Delete</Button>
                    }
                    <Button variant='contained' sx={{ mt: 1 }} onClick={() => cancelUpdate()}>Cancel</Button>
