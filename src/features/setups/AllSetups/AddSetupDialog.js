...
const AddSetupDialog = ({ openAddDialog, handleAddDialogClose }) => {
    ...
    return (
        <Dialog open={openAddDialog} onClose={handleAddDialogClose}>
            ...
            >
                {
                    orderedDocuments ? orderedDocuments.map(({id, name}) =>
                        <DropdownMenuItem onClick={() => handleSelectDocument({id, name})}>{name}</DropdownMenuItem>
                    )
                    : null
                }
            </Menu>
            ...
            <DialogActions sx={{ mb: 1, px: 3 }}>
                <Button onClick={handleAddDialogClose} variant="text">Cancel</Button>
