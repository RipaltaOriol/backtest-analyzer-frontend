...
//   }
    ...
    const handleApply = async () => {
        ...
        if (column[0] === null || action[0] === null || (!values.length && number === undefined)) {
            return handleClose()
        }
        ...
        if (filterOptionIdx !== -1 && setup.options[filterOptionIdx].type === 'number') {
            filter.value = [Number(number)]
        }
        if (filterOptionIdx !== -1 && setup.options[filterOptionIdx].type === 'string') {
            filter.value = values
        }
        try {
        ...
        >
            <Box sx={{ p: '24px' }}>
                ...
                <Select
                    ...
                    renderValue={value => (
                        <>
                            <Typography display='inline-block' sx={{ minWidth: 60, color: "#9AA6AC", fontSize: '14px' }}>Column</Typography>
                            <Typography display='inline' sx={{ fontSize: '14px' }}>{value[1]}</Typography>
                        </>
                    )}
                    onChange={(e, child) => {
                ...
                >
                    {setup ? setup?.options.map((option, idx) => (
                        <FilterMenuItem key={idx} id={idx} value={[option.id, option.name]}>{option.name}</FilterMenuItem>
                    )) : null}
                </Select>
                <Select
                    size="small"
                    ...
                    IconComponent={(props) => (<KeyboardArrowDownRoundedIcon {...props} />)}
                >
                    { filterOptionIdx !== -1 ? setup.options[filterOptionIdx].type === 'number' ?
                        Object.keys(operations.numeric).map((item, idx) => (
                            <FilterMenuItem key={idx} value={[item, operations.numeric[item]]}>{operations.numeric[item]}</FilterMenuItem>
                        ))
                        :
                        Object.keys(operations.string).map((item, idx) => (
                            <FilterMenuItem key={idx} value={[item, operations.string[item]]}>{operations.string[item]}</FilterMenuItem>
                        ))
                    : <></>}
                </Select>
                    ...
                    >
                        { setup.options[filterOptionIdx].values.map((value, idx) => (
                            <FilterMenuItem key={idx} value={value}>
                                <Checkbox checked={values.indexOf(value) > -1} size="small" sx={{py: 0}}/>
