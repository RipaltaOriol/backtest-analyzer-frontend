    ...
    content = orderedDocuments.map((doc) => (
      ...
      >
        ...
        <ListItemSecondaryAction>
          <IconButton onClick={() => handleClickOpen(doc.id)} edge="end" aria-label="rename" sx={{ ml: 2 }}>
            <DriveFileRenameOutlineIcon />
          </IconButton>
          <IconButton onClick={() => cloneDocument({ id: doc.id }) } edge="end" aria-label="copy" sx={{ ml: 2 }}>
            <ContentCopyIcon />
          </IconButton>
          <IconButton onClick={() => deleteDocument({ id: doc.id }) } edge="end" aria-label="delete" sx={{ ml: 2 }}>
            <DeleteIcon />
          </IconButton>
        </ListItemSecondaryAction>
      </ListItem>
  ...
  return (
    <Box>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          <DialogContentText>
            To change the name of the current file write the new name for this document:
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
